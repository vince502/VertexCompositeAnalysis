    ...
    bField_esToken_(iC.esConsumes<MagneticField, IdealMagneticFieldRecord>())
{
  ...
  trk1cosPhiCut = theParameters.getParameter<double>(string("trk1cosPhiCut"));
  trk2cosPhiCut = theParameters.getParameter<double>(string("trk2cosPhiCut"));
  trk12cosPhiCut = theParameters.getParameter<double>(string("trk12cosPhiCut"));
  trk1dRCut = theParameters.getParameter<double>(string("trk1dRCut"));
  trk2dRCut = theParameters.getParameter<double>(string("trk2dRCut"));
  trk12dRCut = theParameters.getParameter<double>(string("trk12dRCut"));
  oniapTCut = theParameters.getParameter<double>(string("oniapTCut"));
  trk1pTCut = theParameters.getParameter<double>(string("trk1pTCut"));
...
//  edm::ValueMap<float>::Filler mvaFiller(*mvaValValueMap);
  ...
  for(unsigned it=0; it<theOnias.size(); ++it){
    ...
    if(!(
	    (theOnia.mass() > 2.70  && theOnia.mass() < 4.00) ||
	    (theOnia.mass() > 9.0  && theOnia.mass() < 10.5) || 
	    false
	  )) continue;
    if(theOnia.pt() < oniapTCut ) continue;
    ...
    for(unsigned int trdx = 0; trdx < theTrackRefsP.size(); trdx++) {
      ...
      if((&trk1TransTkPtr->track())->pt() < trk1pTCut) continue;
      if(
              match_OniaPiPi(&ttk0.track(), &trk1TransTkPtr->track()) ||
              match_OniaPiPi(&ttk1.track(), &trk1TransTkPtr->track()) ) continue;
      double cosDphi1 = cos(Dphi(theOnia.phi(), (&trk1TransTkPtr->track())->phi()));
      double dR1 = reco::deltaR(theOnia.eta(), theOnia.phi(), (&trk1TransTkPtr->track())->eta(), (&trk1TransTkPtr->track())->phi());
      if( cosDphi1 < trk1cosPhiCut || dR1 > trk1dRCut ) continue;
      for(unsigned int trdx2 = 0; trdx2 < theTrackRefsM.size(); trdx2++) {
        ...
        if((&trk2TransTkPtr->track())->pt() < trk2pTCut) continue;
        if( fabs(trk2TransTkPtr->track().eta() - trk1TransTkPtr->track().eta()) > batTrkEtaDiffCut ) continue;

        double cosDphi2 = cos(Dphi(theOnia.phi(), (&trk2TransTkPtr->track())->phi()));
        double dR2 = reco::deltaR(theOnia.eta(), theOnia.phi(), (&trk2TransTkPtr->track())->eta(), (&trk2TransTkPtr->track())->phi());
        if( cosDphi2 < trk2cosPhiCut || dR2 > trk2dRCut ) continue;
        double cosDphi12 = cos(Dphi((&trk1TransTkPtr->track())->phi(), (&trk2TransTkPtr->track())->phi()));
        double dR12 = reco::deltaR((&trk2TransTkPtr->track())->eta(), (&trk2TransTkPtr->track())->phi(), (&trk1TransTkPtr->track())->eta(), (&trk1TransTkPtr->track())->phi());
        if( cosDphi12 < trk12cosPhiCut || dR12 > trk12dRCut ) continue;

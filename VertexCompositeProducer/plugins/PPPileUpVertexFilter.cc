 ...
 * (pileup) which affects flow study. This is performed by looking at the
 * characteristics of the reconstructed vertices. Main structure copied from
 * PAPileUpVertexFilter by Eric and Wei.
 *
...
PPPileUpVertexFilter::PPPileUpVertexFilter(const edm::ParameterSet &iConfig)
    :
      ...
      surfaceFunctionString_(
          iConfig.getParameter<std::string>("surfaceFunctionString")),
      surfaceCutParameters_(
          iConfig.getParameter<std::vector<double>>("surfaceCutParameters")) {

  vtxSrc_ = consumes<reco::VertexCollection>(
  ...
  if ((int)surfaceCutParameters_.size() == func2D_->GetNpar()) {
    for (unsigned int i = 0; i < surfaceCutParameters_.size(); i++)
      func2D_->SetParameter(i, surfaceCutParameters_[i]);
  } else {
    // really need to quit with an error that the configuration is misformed
    for (unsigned int i = 0; i < surfaceCutParameters_.size(); i++)
      func2D_->SetParameter(i, 0);
  }
}
PPPileUpVertexFilter::~PPPileUpVertexFilter() { delete func2D_; }

bool PPwayToSort(reco::Vertex a, reco::Vertex b) {
  return a.tracksSize() > b.tracksSize();
}

bool PPPileUpVertexFilter::filter(edm::Event &iEvent,
  ...
  for (unsigned int i = 1; i < vsorted.size(); i++) {
    ...
    // surfaceCut: filter on PVs above a curved surface of the form Ntrk(dz,
    // NtrkLead)
    if (doSurfaceCut_) {
      if (nTrk > func2D_->Eval(dz, nTrkLead) && dz > surfaceMinDzEval_)
        accepted = false;
      if (nTrk > func2D_->Eval(surfaceMinDzEval_, nTrkLead) &&
          dz <= surfaceMinDzEval_)
        accepted = false;
    }
  }
  return accepted;
}
void PPPileUpVertexFilter::endJob() {}

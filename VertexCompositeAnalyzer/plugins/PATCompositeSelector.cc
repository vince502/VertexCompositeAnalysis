...
PATCompositeSelector::PATCompositeSelector(const edm::ParameterSet &iConfig) {
  ...
  if (iConfig.exists("useExistingMVA"))
    useExistingMVA_ = iConfig.getParameter<bool>("useExistingMVA");
  ...
  if (useAnyMVA_) {
    if (useExistingMVA_ && iConfig.exists("MVACollection")) {
      ...
      if (iConfig.exists("GBRForestLabel"))
...
void PATCompositeSelector::produce(edm::Event &iEvent,
  ...
  //    std::make_unique< pat::CompositeCandidateCollection > theNewV0Cands( new
...
void PATCompositeSelector::fillRECO(edm::Event &iEvent,
                                    const edm::EventSetup &iSetup) {
  // get collections
  ...
  const pat::CompositeCandidateCollection *v0candidates_ =
  ...
  for (unsigned it = 0; it < v0candidates_->size(); ++it) {
    ...
    */
    ...
    if (threeProngDecay_) {
      ...
      if (trkHighPurity_ && !trkquality3)
        continue;
      trkChi3 = dau3->normalizedChi2();
      ptErr3 = dau3->ptError();
      if (ptErr3 / dau3->pt() > trkPtErrMax_)
        continue;
      nhit3 = dau3->numberOfValidHits();
      if (nhit3 < trkNHitMin_)
        continue;
    ...
    if (useAnyMVA_ && useExistingMVA_) {
      mva = (*mvavalues)[it];
      if (mva < mvaMin_ || mva > mvaMax_)
        continue;

      if (mva < GetMVACut(y, pt))
        continue;
      ...
      if (forestLabel_ == "D0InpPb" || forestLabel_ == "D0Inpp" ||
        ...
        //            gbrVals_[20] = bestvz;
        //            gbrVals_[21] = H2dedx2;
      } else if (forestLabel_ == "DsInpPb" || forestLabel_ == "DsInpp" ||
        ...
        gbrVals_[15] = ptErr2;
        gbrVals_[16] = H2dedx2;
      } else if (forestLabel_ == "DiMuInpPb" || forestLabel_ == "DiMuInpp" ||
        ...
        std::cout << "not valid forest label!" << std::endl;
        continue;
      }
...
double PATCompositeSelector::GetMVACut(double y, double pt) {
  double mvacut = -1.0;
  if (fabs(y) > 2.4)
    return 1.0;
...
int PATCompositeSelector::muAssocToTrack(
...
// loop  ------------
void PATCompositeSelector::beginJob() {}

// ------------ method called once each job just after ending the event
// loop  ------------
void PATCompositeSelector::endJob() {}

...
int VertexCompositeNtupleProducer2::muAssocToTrack(const reco::TrackRef &trackref,
...
void VertexCompositeNtupleProducer2::initHistogram() {
  for (unsigned int ipt = 0; ipt < pTBins_.size() - 1; ipt++) {
    for (unsigned int iy = 0; iy < yBins_.size() - 1; iy++) {
      ...
      //  h3DDCAVsMVA[iy][ipt] = fs->make<TH2F>(Form("h3DDCAVsMVA_y%d_pt%d",iy,ipt),";mva;3D
      //  DCA;",100,-1.,1.,1000,0,10); h2DDCAVsMVA[iy][ipt] =
      //  fs->make<TH2F>(Form("h2DDCAVsMVA_y%d_pt%d",iy,ipt),";mva;2D DCA;",100,-1.,1.,1000,0,10);

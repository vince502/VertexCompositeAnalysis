    ...
    for (ushort iTr = 0; iTr < NTRG_; iTr++) {
      ...
      if (trgIdxFound.size() > 1) {
        for (const auto &trgIdx : trgIdxFound) {
          if (triggerResults->accept(trgIdx)) {
            triggerIndex = trgIdx;
            break;
          }
        }
  ...
  for (uint it = 0; it < candSize; ++it) {
    ...
    if (doGenMatching_) {
      ...
      for (ushort iDau = 0; iDau < nDau; iDau++) {
        ...
        if (fabs(recDau->mass() - genDau->mass()) > 0.01) {
          isSwap[it] = true;
        }
      }
...
void PATCompositeTreeProducer::fillGEN(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (uint idx = 0; idx < genpars->size(); ++idx) {
    ...
    if (trk.isNull())
      continue; // check gen particle ref
    if (!hasDaughters(trk, PID_dau_))
      continue; // check if has the daughters
...
// loop  ------------
void PATCompositeTreeProducer::beginJob() {
